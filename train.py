...
def processFiles(pos_dir, neg_dir, recurse=False, output_file=False,
    ...
    # Use pos_train, pos_val, pos_test and neg_train, neg_val, neg_test to represent the Train, Validation and Test sets of Positive and Negtive sets.

    pos_train, pos_test = train_test_split(pos_features, test_size=0.25, random_state=77, shuffle=True)
    pos_train, pos_val = train_test_split(pos_train, test_size=0.2, random_state=77, shuffle=True)
    neg_train, neg_test = train_test_split(neg_features, test_size=0.25, random_state=77, shuffle=True)
    neg_train, neg_val = train_test_split(neg_train, test_size=0.2, random_state=77, shuffle=True)
...
def trainSVM(filepath=None, feature_data=None, C=1,
    ...
    x_train = np.concatenate((feature_data["pos_train"], feature_data["neg_train"]), axis=0)
    x_val = np.concatenate((feature_data['pos_val'], feature_data['neg_val']), axis=0)
    x_test = np.concatenate((feature_data['pos_test'], feature_data['neg_test']), axis=0)

    y_train = np.concatenate((np.ones(len(feature_data["pos_train"])), np.zeros(len(feature_data["neg_train"]))),
                             axis=None)
    y_val = np.concatenate((np.ones(len(feature_data["pos_val"])), np.zeros(len(feature_data["neg_val"]))),
                             axis=None)
    y_test = np.concatenate((np.ones(len(feature_data["pos_test"])), np.zeros(len(feature_data["neg_test"]))),
                             axis=None)
    svc = LinearSVC(C=C, loss=loss, penalty=penalty, dual=dual, fit_intercept=fit_intercept)
